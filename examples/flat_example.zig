...
pub fn main() !void {
    ...
    idle_entry_names[0] = try allocator.dupe(u8, "/ExampleHSM/idle/entry");
    ...
    active_entry_names[0] = try allocator.dupe(u8, "/ExampleHSM/active/entry");
    active_state.entry = active_entry_names;

    const active_exit_names = try allocator.alloc([]const u8, 1);
    active_exit_names[0] = try allocator.dupe(u8, "/ExampleHSM/active/exit");
    ...
    process_effects[0] = try allocator.dupe(u8, "/ExampleHSM/process_effect");
    process_trans.effects = process_effects;

    const complete_trans = hsm.getTransition(&model, "/ExampleHSM/complete_transition").?;
    complete_trans.guard = try allocator.dupe(u8, "/ExampleHSM/completion_guard");
