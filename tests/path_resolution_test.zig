...
const PathTestInstance = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) Self {
        return Self{
            ...
            .navigation_history = std.ArrayList([]const u8).init(allocator),
            .current_path = "none",
            .resolution_count = 0,
            .allocator = allocator,
        };
    ...
    pub fn deinit(self: *Self) void {
        self.navigation_history.deinit();
        self.base.deinit();
    }
    ...
    pub fn recordNavigation(self: *Self, from: []const u8, to: []const u8) void {
        ...
        self.navigation_history.append(navigation) catch unreachable;
    ...
    const model = comptime hsm.define("AbsolutePathTest", .{
        ...
        hsm.state("container", .{
            ...
            hsm.state("start", .{
                ...
                hsm.transition(.{ hsm.on("go_absolute"), hsm.effect(recordAbsoluteNav), hsm.target("/AbsolutePathTest/other/target") })
            }),
            hsm.state("nested", .{
                hsm.entry(siblingEntry),
                hsm.state("deep", .{
                    hsm.entry(deepEntry),
                    hsm.transition(.{ hsm.on("go_root"), hsm.target("/AbsolutePathTest/container/start") })
                })
            })
        }),
        hsm.state("other", .{
            hsm.entry(relativeEntry),
            hsm.state("target", .{
                hsm.entry(absoluteEntry),
...
test "Relative path resolution with current directory" {
    const model = comptime hsm.define("RelativePathTest", .{
        hsm.initial(hsm.target("workspace")),
        hsm.state("workspace", .{
            hsm.entry(parentEntry),
            hsm.initial(hsm.target("folder_a")),
            hsm.state("folder_a", .{
                hsm.entry(childEntry),
                hsm.initial(hsm.target("file1")),
                hsm.state("file1", .{
                    hsm.entry(relativeEntry),
                    hsm.transition(.{ hsm.on("to_file2"), hsm.effect(recordRelativeNav), hsm.target("../file2") }),
                    hsm.transition(.{ hsm.on("to_folder_b"), hsm.target("../../folder_b/file3") })
                }),
                hsm.state("file2", .{
                    hsm.entry(siblingEntry),
                    hsm.transition(.{ hsm.on("to_subfolder"), hsm.target("./subfolder/nested_file") })
                }),
                hsm.state("subfolder", .{
                    hsm.entry(deepEntry),
                    hsm.state("nested_file", .{
                        hsm.entry(absoluteEntry)
                    })
                })
            }),
            hsm.state("folder_b", .{
                ...
                hsm.state("file3", .{
                    hsm.entry(relativeEntry),
                    hsm.transition(.{ hsm.on("back"), hsm.target("../../folder_a/file1") })
                })
            })
        })
    });
...
test "Parent directory path resolution" {
    const model = comptime hsm.define("ParentPathTest", .{
        hsm.initial(hsm.target("root")),
        hsm.state("root", .{
            hsm.entry(parentEntry),
            ...
            hsm.state("level1", .{
                ...
                hsm.state("level2", .{
                    ...
                    hsm.state("level3", .{
                        hsm.entry(deepEntry),
                        hsm.transition(.{ hsm.on("up_one"), hsm.effect(recordParentNav), hsm.target("..") }),
                        hsm.transition(.{ hsm.on("up_two"), hsm.target("../..") }),
                        hsm.transition(.{ hsm.on("up_three"), hsm.target("../../..") }),
                        hsm.transition(.{ hsm.on("to_uncle"), hsm.target("../../level1_sibling") })
                    })
                }),
                hsm.state("level2_sibling", .{
                    hsm.entry(absoluteEntry)
                })
            }),
            hsm.state("level1_sibling", .{
                hsm.entry(relativeEntry),
                hsm.transition(.{ hsm.on("to_nephew"), hsm.target("../level1/level2/level3") })
            })
        })
    });
...
test "Self-reference path resolution" {
    const model = comptime hsm.define("SelfPathTest", .{
        hsm.initial(hsm.target("stateful")),
        hsm.state("stateful", .{
            hsm.entry(parentEntry),
            hsm.initial(hsm.target("inner")),
            hsm.transition(.{ hsm.on("reset_self"), hsm.effect(recordSelfNav), hsm.target(".") }),
            hsm.state("inner", .{
                hsm.entry(childEntry),
                hsm.transition(.{ hsm.on("reset_inner"), hsm.target(".") }),
                hsm.transition(.{ hsm.on("reset_parent"), hsm.target("..") })
            })
        }),
        hsm.state("other", .{
            hsm.entry(siblingEntry),
            hsm.transition(.{ hsm.on("to_stateful"), hsm.target("../stateful") })
        })
    });
    ...
    // Self-transition on parent state
    try sm.dispatch(&context, hsm.Event.init(testing.allocator, "reset_parent"));
    try testing.expect(std.mem.endsWith(u8, sm.state(), "inner")); // Should re-enter initial
    try testing.expectEqualStrings("child", instance.current_path);
        ...
        hsm.state("app", .{
            ...
            hsm.state("main", .{
                ...
                hsm.state("editor", .{
                    ...
                    hsm.state("document", .{
                        hsm.entry(relativeEntry),
                        hsm.transition(.{ hsm.on("absolute_nav"), hsm.target("/MixedPathTest/sidebar/tools/palette") }),
                        hsm.transition(.{ hsm.on("relative_nav"), hsm.target("../../settings/preferences") }),
                        hsm.transition(.{ hsm.on("parent_nav"), hsm.target("../../../sidebar") })
                    })
                }),
                hsm.state("settings", .{
                    hsm.entry(deepEntry),
                    hsm.state("preferences", .{
                        hsm.entry(absoluteEntry),
                        hsm.transition(.{ hsm.on("self_reset"), hsm.target(".") }),
                        hsm.transition(.{ hsm.on("back_to_doc"), hsm.target("../../editor/document") })
                    })
                })
            }),
            hsm.state("sidebar", .{
                hsm.entry(childEntry),
                hsm.initial(hsm.target("tools")),
                hsm.state("tools", .{
                    hsm.entry(siblingEntry),
                    hsm.state("palette", .{
                        hsm.entry(deepEntry),
                        hsm.transition(.{ hsm.on("close"), hsm.target("/MixedPathTest/app/main/editor/document") })
                    })
                })
            })
        })
...
test "Path resolution edge cases and error conditions" {
    ...
    const model = comptime hsm.define("EdgeCasePathTest", .{
        hsm.initial(hsm.target("container")),
        hsm.state("container", .{
            hsm.entry(parentEntry),
            ...
            hsm.state("state", .{
                hsm.entry(childEntry),
                hsm.transition(.{ hsm.on("to_nested_state"), hsm.target("../nested/state") }),
                hsm.transition(.{ hsm.on("to_deep_state"), hsm.target("../nested/deep/state") })
            }),
            hsm.state("nested", .{
                hsm.entry(siblingEntry),
                hsm.state("state", .{
                    hsm.entry(relativeEntry),
                    hsm.transition(.{ hsm.on("to_parent_state"), hsm.target("../../state") })
                }),
                hsm.state("deep", .{
                    hsm.entry(deepEntry),
                    hsm.state("state", .{
    ...
    try testing.expect(std.mem.contains(u8, sm.state(), "container"));
    try testing.expectEqualStrings("child", instance.current_path);
    
    // Navigate to nested state with same name
    try sm.dispatch(&context, hsm.Event.init(testing.allocator, "to_nested_state"));
    try testing.expect(std.mem.endsWith(u8, sm.state(), "state"));
    try testing.expect(std.mem.contains(u8, sm.state(), "nested"));
    try testing.expectEqualStrings("relative", instance.current_path);
    ...
    try testing.expect(std.mem.endsWith(u8, sm.state(), "state"));
    try testing.expect(std.mem.contains(u8, sm.state(), "deep"));
    try testing.expectEqualStrings("absolute", instance.current_path);
