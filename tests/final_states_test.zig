...
const FinalStateTestInstance = struct {
    ...
    pub fn deinit(self: *Self) void {
        self.final_states_reached.deinit(self.allocator);
        self.invalid_operations.deinit(self.allocator);
        self.base.deinit();
    }

    pub fn recordCompletion(self: *Self, final_state: []const u8) void {
        self.completion_count += 1;
        self.final_states_reached.append(self.allocator, final_state) catch unreachable;
    }
    ...
    pub fn recordInvalidOperation(self: *Self, operation: []const u8) void {
        self.invalid_operations.append(self.allocator, operation) catch unreachable;
    }
};
...
test "Basic final state termination" {
    const model = comptime hsm.define("BasicFinalTest", .{ hsm.initial(hsm.target("start")), hsm.state("start", .{hsm.transition(.{ hsm.on("complete"), hsm.effect(completionEffect), hsm.target("finished") })}), hsm.final("finished") });
    ...
    try testing.expect(std.mem.indexOf(u8, sm.state(), "finished") != null);
}

test "Multiple final states with different completion paths" {
    const model = comptime hsm.define("MultipleFinalTest", .{ hsm.initial(hsm.target("processing")), hsm.state("processing", .{ hsm.transition(.{ hsm.on("succeed"), hsm.effect(successEffect), hsm.target("success") }), hsm.transition(.{ hsm.on("fail"), hsm.effect(failureEffect), hsm.target("failure") }), hsm.transition(.{ hsm.on("cancel"), hsm.target("cancelled") }) }), hsm.final("success"), hsm.final("failure"), hsm.final("cancelled") });
...
test "Final states in hierarchical state machines" {
    const model = comptime hsm.define("HierarchicalFinalTest", .{ hsm.initial(hsm.target("parent")), hsm.state("parent", .{ hsm.initial(hsm.target("child")), hsm.state("child", .{ hsm.transition(.{ hsm.on("finish_child"), hsm.target("../child_done") }), hsm.transition(.{ hsm.on("finish_parent"), hsm.target("../../parent_done") }) }), hsm.final("child_done") }), hsm.final("parent_done") });
    ...
    // Test child-level final state
    {
        ...
        try testing.expect(std.mem.indexOf(u8, sm.state(), "parent") != null);
    }
...
test "Final state restrictions - no entry/exit actions" {
    ...
    const minimal_final_model = comptime hsm.define("MinimalFinalTest", .{
        ...
        hsm.state("start", .{hsm.transition(.{ hsm.on("end"), hsm.target("end") })}),
...
test "Final state restrictions - no activities" {
    ...
    const simple_completion_model = comptime hsm.define("SimpleCompletionTest", .{
        ...
        hsm.state("working", .{hsm.transition(.{ hsm.on("complete"), hsm.target("completed") })}),
...
test "Conditional transitions to final states" {
    const model = comptime hsm.define("ConditionalFinalTest", .{ hsm.initial(hsm.target("evaluator")), hsm.state("evaluator", .{ hsm.transition(.{ hsm.on("try_complete"), hsm.guard(shouldComplete), hsm.effect(completionEffect), hsm.target("success") }), hsm.transition(.{ hsm.on("try_complete"), hsm.effect(transitionAttemptEffect), hsm.target("retry") }) }), hsm.state("retry", .{hsm.transition(.{ hsm.on("try_again"), hsm.target("../evaluator") })}), hsm.final("success") });
...
test "Final states with complex completion workflows" {
    const model = comptime hsm.define("ComplexCompletionTest", .{ hsm.initial(hsm.target("workflow")), hsm.state("workflow", .{ hsm.initial(hsm.target("step1")), hsm.state("step1", .{ hsm.transition(.{ hsm.on("next"), hsm.target("../step2") }), hsm.transition(.{ hsm.on("abort"), hsm.target("../../aborted") }) }), hsm.state("step2", .{ hsm.transition(.{ hsm.on("next"), hsm.target("../step3") }), hsm.transition(.{ hsm.on("back"), hsm.target("../step1") }), hsm.transition(.{ hsm.on("abort"), hsm.target("../../aborted") }) }), hsm.state("step3", .{ hsm.transition(.{ hsm.on("finish"), hsm.effect(successEffect), hsm.target("../../completed") }), hsm.transition(.{ hsm.on("back"), hsm.target("../step2") }), hsm.transition(.{ hsm.on("abort"), hsm.target("../../aborted") }) }) }), hsm.final("completed"), hsm.final("aborted") });
