...
test "Choice state with multiple guards" {
    const model = comptime hsm.define("ChoiceTest", .{
        ...
        hsm.state("input", .{
            hsm.transition(.{ hsm.on("set_value"), hsm.effect(setValue), hsm.target("router") })
        }),
...
test "Nested choice states" {
    const model = comptime hsm.define("NestedChoiceTest", .{
        hsm.initial(hsm.target("container")),
        
        hsm.state("container", .{
            hsm.initial(hsm.target("first_choice")),
            
            hsm.choice("first_choice", .{
                hsm.transition(.{ hsm.guard(isLow), hsm.target("../second_choice") }),
                hsm.transition(.{ hsm.target("../../outer_state") })
            }),
            
            hsm.choice("second_choice", .{
                hsm.transition(.{ hsm.guard(isLow), hsm.target("../inner_state") }),
                hsm.transition(.{ hsm.target("../../outer_state") })
            }),
            
            hsm.state("inner_state", .{})
        }),
        
        hsm.state("outer_state", .{})
    });
