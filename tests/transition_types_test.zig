...
const TransitionTestInstance = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) Self {
        return Self{
            ...
            .transition_count = 0,
            .transition_sequence = std.ArrayList([]const u8).init(allocator),
            .entry_count = 0,
            .exit_count = 0,
            .effect_count = 0,
            .entry_sequence = std.ArrayList([]const u8).init(allocator),
            .exit_sequence = std.ArrayList([]const u8).init(allocator),
            .effect_sequence = std.ArrayList([]const u8).init(allocator),
            .allocator = allocator,
        };
    }
    
    pub fn deinit(self: *Self) void {
        self.transition_sequence.deinit();
        self.entry_sequence.deinit();
        self.exit_sequence.deinit();
        self.effect_sequence.deinit();
        self.base.deinit();
    }
    
    pub fn recordTransition(self: *Self, name: []const u8) void {
        self.transition_count += 1;
        self.transition_sequence.append(name) catch unreachable;
    }
    
    pub fn recordEntry(self: *Self, name: []const u8) void {
        self.entry_count += 1;
        self.entry_sequence.append(name) catch unreachable;
    }
    ...
    pub fn recordEffect(self: *Self, name: []const u8) void {
        self.effect_count += 1;
        self.effect_sequence.append(name) catch unreachable;
    }
};
...
test "External transitions with entry/exit actions" {
    const model = comptime hsm.define("ExternalTransitionTest", .{
        hsm.initial(hsm.target("state_a")),
        hsm.state("state_a", .{
            hsm.entry(entryA),
            hsm.exit(exitA),
            hsm.transition(.{ hsm.on("go_to_b"), hsm.effect(effectExternal), hsm.target("../state_b") })
        }),
        hsm.state("state_b", .{
            hsm.entry(entryB),
            hsm.exit(exitB),
            hsm.transition(.{ hsm.on("go_to_a"), hsm.effect(effectExternal), hsm.target("../state_a") })
        })
    });
    ...
    const model = comptime hsm.define("LocalTransitionTest", .{
        ...
        hsm.state("parent", .{
            ...
            hsm.state("child_a", .{
                hsm.entry(entryA),
                hsm.exit(exitA),
                hsm.transition(.{ hsm.on("switch"), hsm.target("../child_b") })
            }),
            hsm.state("child_b", .{
                hsm.entry(entryB),
                hsm.exit(exitB),
                hsm.transition(.{ hsm.on("switch"), hsm.target("../child_a") })
            })
        }),
        hsm.state("external", .{
            hsm.entry(entryChild)
        })
    });
        ...
        hsm.state("root", .{
            ...
            hsm.state("branch_a", .{
                ...
                hsm.state("leaf_a1", .{
                    hsm.entry(entryChild),
                    hsm.exit(exitChild),
                    hsm.transition(.{ hsm.on("to_a2"), hsm.target("../leaf_a2") }),
                    hsm.transition(.{ hsm.on("to_b"), hsm.target("../../branch_b/leaf_b1") })
                }),
                hsm.state("leaf_a2", .{
                    hsm.entry(entryB),
                    hsm.exit(exitB)
...
test "Transition types with multiple effects and complex paths" {
    ...
    const model = comptime hsm.define("ComplexTransitionTest", .{
        ...
        hsm.state("other_state", .{
            hsm.entry(entryB),
            hsm.exit(exitB),
            hsm.transition(.{ hsm.on("back"), hsm.target("../multi_state") })
        })
    });
