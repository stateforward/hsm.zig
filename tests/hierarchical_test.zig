...
test "Hierarchical state transitions" {
    const model = comptime hsm.define("HierarchicalTest", .{
        ...
        hsm.state("outer", .{
            ...
            // Nested states
            hsm.state("inner1", .{
                hsm.entry(enterInner),
                hsm.exit(exitInner),
                hsm.transition(.{ hsm.on("next"), hsm.target("../inner2") })
            }),
            
            hsm.state("inner2", .{
                hsm.entry(enterInner),
                hsm.exit(exitInner),
                hsm.transition(.{ hsm.on("back"), hsm.target("../inner1") }),
                hsm.transition(.{ hsm.on("out"), hsm.target("../../standalone") })
            }),
        ...
        hsm.state("standalone", .{
            hsm.transition(.{ hsm.on("enter"), hsm.target("../outer") })
        })
    });
...
test "Path resolution" {
    const model = comptime hsm.define("PathTest", .{
        ...
        hsm.state("level1", .{
            ...
            hsm.state("level2a", .{
                hsm.transition(.{ hsm.on("sibling"), hsm.target("../level2b") }),
                hsm.transition(.{ hsm.on("absolute"), hsm.target("/PathTest/level1/level2b") }),
                hsm.transition(.{ hsm.on("self"), hsm.target(".") })
            }),
            
            hsm.state("level2b", .{
                hsm.transition(.{ hsm.on("up"), hsm.target("../../other") }),
                hsm.transition(.{ hsm.on("down"), hsm.target("level3") }),
                
                hsm.state("level3", .{
                    hsm.transition(.{ hsm.on("top"), hsm.target("/PathTest/other") })
                })
            })
        }),
