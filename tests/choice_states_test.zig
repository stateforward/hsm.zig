...
const ChoiceTestInstance = struct {
    ...
    pub fn deinit(self: *Self) void {
        self.guard_evaluations.deinit(self.allocator);
        self.base.deinit();
    }

    pub fn recordGuardEvaluation(self: *Self, guard_name: []const u8) void {
        self.guard_evaluations.append(self.allocator, guard_name) catch unreachable;
    }
...
test "Basic choice state with guards" {
    const model = comptime hsm.define("BasicChoiceTest", .{
        ...
        hsm.state("input", .{hsm.transition(.{ hsm.on("set_value"), hsm.effect(setValue), hsm.target("decision") })}),
        hsm.choice("decision", .{
...
test "Multiple choice states in sequence" {
    const model = comptime hsm.define("SequentialChoiceTest", .{ hsm.initial(hsm.target("start")), hsm.state("start", .{hsm.transition(.{ hsm.on("begin"), hsm.target("first_choice") })}), hsm.choice("first_choice", .{ hsm.transition(.{ hsm.guard(flagGuard), hsm.target("../second_choice") }), hsm.transition(.{hsm.target("../direct_end")}) }), hsm.choice("second_choice", .{ hsm.transition(.{ hsm.guard(lowValueGuard), hsm.effect(markLowRoute), hsm.target("../low_end") }), hsm.transition(.{ hsm.effect(markFallbackRoute), hsm.target("../high_end") }) }), hsm.state("direct_end", .{}), hsm.state("low_end", .{}), hsm.state("high_end", .{}) });
...
test "Choice states with complex guard combinations" {
    const model = comptime hsm.define("ComplexChoiceTest", .{ hsm.initial(hsm.target("setup")), hsm.state("setup", .{hsm.transition(.{ hsm.on("evaluate"), hsm.target("complex_choice") })}), hsm.choice("complex_choice", .{ hsm.transition(.{ hsm.guard(flagGuard), hsm.guard(lowValueGuard), hsm.effect(markLowRoute), hsm.target("../flag_and_low") }), hsm.transition(.{ hsm.guard(flagGuard), hsm.guard(highValueGuard), hsm.effect(markHighRoute), hsm.target("../flag_and_high") }), hsm.transition(.{ hsm.guard(counterGuard), hsm.effect(markMidRoute), hsm.target("../counter_state") }), hsm.transition(.{ hsm.effect(markFallbackRoute), hsm.target("../default_state") }) }), hsm.state("flag_and_low", .{}), hsm.state("flag_and_high", .{}), hsm.state("counter_state", .{}), hsm.state("default_state", .{}) });
...
test "Choice states in hierarchical structures" {
    const model = comptime hsm.define("HierarchicalChoiceTest", .{ hsm.initial(hsm.target("parent")), hsm.state("parent", .{ hsm.initial(hsm.target("child_choice")), hsm.choice("child_choice", .{ hsm.transition(.{ hsm.guard(flagGuard), hsm.target("../nested/deep_state") }), hsm.transition(.{hsm.target("../simple_child")}) }), hsm.state("nested", .{hsm.state("deep_state", .{hsm.transition(.{ hsm.on("back"), hsm.target("../../parent_choice") })})}), hsm.state("simple_child", .{hsm.transition(.{ hsm.on("up"), hsm.target("../parent_choice") })}), hsm.choice("parent_choice", .{ hsm.transition(.{ hsm.guard(lowValueGuard), hsm.target("../../other") }), hsm.transition(.{hsm.target("../simple_child")}) }) }), hsm.state("other", .{}) });
...
test "Choice states with event data in guards" {
    ...
    const model = comptime hsm.define("EventDataChoiceTest", .{ hsm.initial(hsm.target("start")), hsm.state("start", .{hsm.transition(.{ hsm.on("check"), hsm.target("data_choice") })}), hsm.choice("data_choice", .{ hsm.transition(.{ hsm.guard(eventDataGuard.func), hsm.effect(markHighRoute), hsm.target("../above_threshold") }), hsm.transition(.{ hsm.effect(markLowRoute), hsm.target("../below_threshold") }) }), hsm.state("above_threshold", .{}), hsm.state("below_threshold", .{}) });
...
test "Choice states with context cancellation" {
    ...
    const model = comptime hsm.define("CancellationChoiceTest", .{ hsm.initial(hsm.target("start")), hsm.state("start", .{hsm.transition(.{ hsm.on("evaluate"), hsm.target("cancel_choice") })}), hsm.choice("cancel_choice", .{ hsm.transition(.{ hsm.guard(cancelAwareGuard), hsm.effect(markHighRoute), hsm.target("../success") }), hsm.transition(.{ hsm.effect(markFallbackRoute), hsm.target("../fallback") }) }), hsm.state("success", .{}), hsm.state("fallback", .{}) });
