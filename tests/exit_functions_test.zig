...
test "Single exit function execution" {
    const model = comptime hsm.define("SingleExitTest", .{ hsm.initial(hsm.target("state1")), hsm.state("state1", .{ hsm.exit(firstExit), hsm.transition(.{ hsm.on("next"), hsm.target("state2") }) }), hsm.state("state2", .{ hsm.exit(secondExit), hsm.transition(.{ hsm.on("back"), hsm.target("state1") }) }) });
...
test "Multiple exit functions execution order" {
    const model = comptime hsm.define("MultipleExitTest", .{ hsm.initial(hsm.target("complex_state")), hsm.state("complex_state", .{ hsm.exit(.{ saveStateExit, cleanupExit, finalizeExit }), hsm.transition(.{ hsm.on("shutdown"), hsm.target("simple_state") }) }), hsm.state("simple_state", .{ hsm.exit(.{ firstExit, secondExit }), hsm.transition(.{ hsm.on("restart"), hsm.target("complex_state") }) }) });
...
test "Exit functions with event data access" {
    ...
    const model = comptime hsm.define("ExitDataTest", .{ hsm.initial(hsm.target("start")), hsm.state("start", .{ hsm.exit(dataExit), hsm.transition(.{ hsm.on("with_data"), hsm.target("end") }) }), hsm.final("end") });
...
test "Exit functions in hierarchical states" {
    const model = comptime hsm.define("HierarchicalExitTest", .{ hsm.initial(hsm.target("parent")), hsm.state("parent", .{ hsm.exit(firstExit), hsm.initial(hsm.target("child")), hsm.state("child", .{ hsm.exit(secondExit), hsm.transition(.{ hsm.on("up"), hsm.target("../../other") }) }) }), hsm.state("other", .{}) });
...
test "Exit functions with self transitions" {
    const model = comptime hsm.define("SelfTransitionExitTest", .{ hsm.initial(hsm.target("start")), hsm.state("start", .{ hsm.exit(firstExit), hsm.transition(.{ hsm.on("self"), hsm.target(".") }), hsm.transition(.{ hsm.on("next"), hsm.target("end") }) }), hsm.final("end") });
...
test "Exit functions with context cancellation" {
    ...
    const model = comptime hsm.define("ExitCancellationTest", .{ hsm.initial(hsm.target("start")), hsm.state("start", .{ hsm.exit(cancelCheckExit), hsm.transition(.{ hsm.on("next"), hsm.target("middle") }) }), hsm.state("middle", .{ hsm.exit(cancelCheckExit), hsm.transition(.{ hsm.on("next"), hsm.target("end") }) }), hsm.final("end") });
...
test "Exit functions state cleanup pattern" {
    ...
    const model = comptime hsm.define("ResourceCleanupTest", .{ hsm.initial(hsm.target("idle")), hsm.state("idle", .{hsm.transition(.{ hsm.on("acquire"), hsm.effect(allocateResource.func), hsm.target("holding") })}), hsm.state("holding", .{ hsm.exit(.{ deallocateResource.func, finalizeExit }), hsm.transition(.{ hsm.on("release"), hsm.target("idle") }) }) });
