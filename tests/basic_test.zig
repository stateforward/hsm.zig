...
test "Basic state machine functionality" {
    const model = comptime hsm.define("BasicTest", .{ hsm.initial(hsm.target("start")), hsm.state("start", .{ hsm.entry(setFlag), hsm.transition(.{ hsm.on("next"), hsm.effect(incrementValue), hsm.target("middle") }) }), hsm.state("middle", .{ hsm.entry(incrementValue), hsm.transition(.{ hsm.on("next"), hsm.effect(incrementValue), hsm.target("end") }), hsm.transition(.{ hsm.on("back"), hsm.target("../start") }) }), hsm.final("end") });
...
test "Guard conditions" {
    ...
    // Reset to waiting
    const model2 = comptime hsm.define("GuardTest2", .{ hsm.initial(hsm.target("waiting")), hsm.state("waiting", .{ hsm.transition(.{ hsm.on("check"), hsm.guard(checkValue), hsm.target("passed") }), hsm.transition(.{ hsm.on("check"), hsm.target("failed") }), hsm.transition(.{ hsm.on("increment"), hsm.effect(incrementValue) }) }), hsm.state("passed", .{}), hsm.state("failed", .{}) });
...
test "Multiple entry and exit actions" {
    const model = comptime hsm.define("MultiActionTest", .{ hsm.initial(hsm.target("state1")), hsm.state("state1", .{ hsm.entry(.{ setFlag, incrementValue }), hsm.exit(.{ clearFlag, incrementValue }), hsm.transition(.{ hsm.on("next"), hsm.target("state2") }) }), hsm.state("state2", .{}) });
