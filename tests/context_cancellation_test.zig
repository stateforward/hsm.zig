...
const CancellationTestInstance = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) Self {
        return Self{
            ...
            .cleanup_calls = 0,
            .operation_results = std.ArrayList([]const u8).init(allocator),
    ...
    pub fn recordResult(self: *Self, result: []const u8) void {
        self.operation_results.append(result) catch unreachable;
    }
};
...
test "Basic context cancellation in activities" {
    const model = comptime hsm.define("BasicCancellationTest", .{
        hsm.initial(hsm.target("active")),
        hsm.state("active", .{
            hsm.activity(cancellableActivity),
...
test "Context cancellation during state machine startup" {
    const model = comptime hsm.define("StartupCancellationTest", .{
        hsm.initial(hsm.target("startup")),
        hsm.state("startup", .{
            hsm.entry(cancellationAwareEntry),
            hsm.exit(cancellationAwareExit),
            hsm.initial(hsm.target("nested")),
            hsm.state("nested", .{
                hsm.entry(cancellationAwareEntry),
                hsm.activity(immediateActivity)
            })
        })
    });
    ...
    var found_entry_cancel = false;
    for (instance.operation_results.items) |result| {
        if (std.mem.eql(u8, result, "entry_normal")) {
            // Some entries might complete before cancellation is detected
        }
    }
...
test "Multiple concurrent activities with cancellation" {
    ...
    const model = comptime hsm.define("ConcurrentCancellationTest", .{
        hsm.initial(hsm.target("concurrent")),
        hsm.state("concurrent", .{
            hsm.activity(.{ ConcurrentActivity1, ConcurrentActivity2, longRunningActivity }),
...
test "Cancellation during hierarchical state transitions" {
    const model = comptime hsm.define("HierarchicalCancellationTest", .{
        ...
        hsm.state("parent", .{
            ...
            hsm.state("child", .{
                hsm.entry(cancellationAwareEntry),
                hsm.exit(cancellationAwareExit),
                hsm.activity(longRunningActivity),
                hsm.transition(.{ hsm.on("move"), hsm.target("../sibling") })
            }),
            hsm.state("sibling", .{
                hsm.entry(cancellationAwareEntry),
                hsm.exit(cleanupExit),
                hsm.activity(cancellableActivity)
            })
        }),
        hsm.state("external", .{
...
test "Guards and effects with context cancellation" {
    const model = comptime hsm.define("GuardCancellationTest", .{
        hsm.initial(hsm.target("guarded")),
        hsm.state("guarded", .{
            hsm.entry(cancellationAwareEntry),
            hsm.transition(.{ hsm.on("test"), hsm.guard(cancellationGuard), hsm.effect(cancellationAwareEntry), hsm.target("passed") }),
            hsm.transition(.{ hsm.on("test"), hsm.effect(cancellationAwareEntry), hsm.target("failed") })
        }),
        hsm.state("passed", .{
            hsm.entry(cancellationAwareEntry)
        }),
    ...
    // Test with cancelled context
    {
        ...
        try testing.expect(instance.cancellation_count >= 1);
        try testing.expectEqualStrings("guard_cancelled", instance.last_cancellation_reason);
    }
}
...
test "Cleanup during cancellation" {
    ...
    const model = comptime hsm.define("CleanupCancellationTest", .{
        hsm.initial(hsm.target("resource_state")),
        hsm.state("resource_state", .{
            hsm.activity(ResourceActivity),
            hsm.exit(cleanupExit),
            hsm.transition(.{ hsm.on("release"), hsm.target("released") })
        }),
        hsm.state("released", .{})
    });
...
test "Cancellation propagation timing" {
    ...
    const model = comptime hsm.define("TimingCancellationTest", .{
        hsm.initial(hsm.target("timed")),
        hsm.state("timed", .{
            hsm.activity(TimedActivity.func)
        })
    });
