...
const HierarchicalTestInstance = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) Self {
        return Self{
            ...
            .depth_level = 0,
            .state_path = std.ArrayList([]const u8).init(allocator),
            .entry_depth = 0,
            .exit_depth = 0,
            .max_depth_reached = 0,
            .transition_count = 0,
    ...
    pub fn recordEntry(self: *Self, state_name: []const u8, depth: i32) void {
        ...
        self.state_path.append(state_name) catch unreachable;
    }
...
test "Deep nested hierarchy initialization" {
    const model = comptime hsm.define("DeepHierarchyTest", .{
...
test "Multi-branch hierarchy with sibling navigation" {
    const model = comptime hsm.define("MultiBranchTest", .{
        hsm.initial(hsm.target("main")),
        hsm.state("main", .{
            hsm.entry(level1Entry),
            hsm.exit(level1Exit),
            hsm.initial(hsm.target("branch_a")),
            hsm.state("branch_a", .{
                hsm.entry(level2Entry),
                hsm.exit(level2Exit),
                hsm.initial(hsm.target("leaf_a1")),
                hsm.state("leaf_a1", .{
                    hsm.entry(level3Entry),
                    hsm.exit(level3Exit),
                    hsm.transition(.{ hsm.on("to_a2"), hsm.effect(transitionEffect), hsm.target("../leaf_a2") }),
                    hsm.transition(.{ hsm.on("to_b"), hsm.effect(transitionEffect), hsm.target("../../branch_b/leaf_b1") })
                }),
                hsm.state("leaf_a2", .{
                    hsm.entry(level4Entry),
                    hsm.exit(level4Exit),
            ...
            hsm.state("branch_b", .{
                ...
                hsm.state("leaf_b1", .{
                    hsm.entry(level3Entry),
                    hsm.exit(level3Exit),
                    hsm.transition(.{ hsm.on("to_a"), hsm.effect(transitionEffect), hsm.target("../../branch_a/leaf_a1") })
                })
            })
        }),
        hsm.state("other_root", .{
            hsm.entry(level1Entry),
            hsm.exit(level1Exit)
        })
    });
...
test "Hierarchical states with overlapping names" {
    const model = comptime hsm.define("OverlappingNamesTest", .{
        ...
        hsm.state("container_a", .{
            ...
            hsm.state("state", .{
                hsm.entry(level2Entry),
                hsm.exit(level2Exit),
                hsm.transition(.{ hsm.on("to_b"), hsm.effect(transitionEffect), hsm.target("../../container_b/state") })
            })
        }),
        hsm.state("container_b", .{
            ...
            hsm.state("state", .{
                hsm.entry(level2Entry),
                hsm.exit(level2Exit),
                hsm.transition(.{ hsm.on("to_a"), hsm.effect(transitionEffect), hsm.target("../../container_a/state") })
            })
        })
    });
    ...
    try testing.expect(std.mem.contains(u8, sm.state(), "container_a"));
    try testing.expect(std.mem.endsWith(u8, sm.state(), "state"));
    
    // Transition to container_b/state
    try sm.dispatch(&context, hsm.Event.init(testing.allocator, "to_b"));
    try testing.expect(std.mem.contains(u8, sm.state(), "container_b"));
    try testing.expect(std.mem.endsWith(u8, sm.state(), "state"));
    try testing.expect(instance.transition_count == 1);
...
test "Complex hierarchy with choice states and guards" {
    ...
    const model = comptime hsm.define("ComplexHierarchyTest", .{
        hsm.initial(hsm.target("root")),
        hsm.state("root", .{
            hsm.entry(level1Entry),
            hsm.exit(level1Exit),
            hsm.initial(hsm.target("intermediate")),
            hsm.state("intermediate", .{
                hsm.entry(level2Entry),
                hsm.exit(level2Exit),
                hsm.initial(hsm.target("deep")),
                hsm.state("deep", .{
                    hsm.entry(level3Entry),
                    hsm.exit(level3Exit),
                    hsm.transition(.{ hsm.on("navigate"), hsm.target("../decision") })
                }),
                hsm.choice("decision", .{
                    hsm.transition(.{ hsm.guard(depthGuard), hsm.effect(transitionEffect), hsm.target("../../../shallow") }),
                    hsm.transition(.{ hsm.effect(transitionEffect), hsm.target("../deep") })
                })
            }),
            hsm.state("shallow", .{
                hsm.entry(level2Entry),
                hsm.exit(level2Exit),
                hsm.transition(.{ hsm.on("go_deep"), hsm.effect(transitionEffect), hsm.target("../intermediate/deep") })
            })
        }),
        hsm.state("shallow", .{
            hsm.entry(level1Entry),
            ...
            hsm.state("main_panel", .{
                ...
                hsm.state("editor", .{
                    ...
                    hsm.state("document", .{
                        hsm.entry(level4Entry),
                        hsm.exit(level4Exit),
                        hsm.transition(.{ hsm.on("switch_panel"), hsm.target("../../../side_panel/tool") })
                    })
                })
            }),
            hsm.state("side_panel", .{
                hsm.entry(level2Entry),
                hsm.exit(level2Exit),
                hsm.initial(hsm.target("tool")),
                hsm.state("tool", .{
                    hsm.entry(level3Entry),
                    hsm.exit(level3Exit),
                    hsm.transition(.{ hsm.on("switch_back"), hsm.target("../../main_panel/editor/document") })
                })
            })
        })
    });
...
test "Maximum depth hierarchy stress test" {
    const model = comptime hsm.define("MaxDepthTest", .{
        hsm.initial(hsm.target("d1")),
        hsm.state("d1", .{
            hsm.entry(level1Entry),
            hsm.exit(level1Exit),
            hsm.initial(hsm.target("d2")),
            hsm.transition(.{ hsm.on("escape"), hsm.effect(transitionEffect), hsm.target("../escape") }),
            hsm.state("d2", .{
                hsm.entry(level2Entry),
                hsm.exit(level2Exit),
                hsm.initial(hsm.target("d3")),
                hsm.state("d3", .{
                    hsm.entry(level3Entry),
                    hsm.exit(level3Exit),
                    hsm.initial(hsm.target("d4")),
                    hsm.state("d4", .{
                        hsm.entry(level4Entry),
                        hsm.exit(level4Exit),
                        hsm.transition(.{ hsm.on("bubble_up"), hsm.effect(transitionEffect), hsm.target("../../../../escape") })
                    })
                })
            })
        }),
