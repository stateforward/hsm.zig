...
const EventDataTestInstance = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) Self {
        return Self{
            ...
            .received_values = std.ArrayList(i32).init(allocator),
            .received_strings = std.ArrayList([]const u8).init(allocator),
            .received_booleans = std.ArrayList(bool).init(allocator),
            .data_access_count = 0,
    ...
    pub fn deinit(self: *Self) void {
        self.received_values.deinit();
        self.received_strings.deinit();
        self.received_booleans.deinit();
        self.base.deinit();
    }
    
    pub fn recordValue(self: *Self, value: i32) void {
        self.received_values.append(value) catch unreachable;
        self.data_access_count += 1;
    }
    
    pub fn recordString(self: *Self, str: []const u8) void {
        self.received_strings.append(str) catch unreachable;
        self.data_access_count += 1;
    }
    
    pub fn recordBoolean(self: *Self, boolean: bool) void {
        self.received_booleans.append(boolean) catch unreachable;
        self.data_access_count += 1;
    }
...
test "Basic event data types - integers, strings, booleans" {
    const model = comptime hsm.define("BasicDataTest", .{
        ...
        hsm.state("waiting", .{
            hsm.transition(.{ hsm.on("int_event"), hsm.target("int_state") }),
            hsm.transition(.{ hsm.on("string_event"), hsm.target("string_state") }),
            hsm.transition(.{ hsm.on("bool_event"), hsm.target("bool_state") })
        }),
        hsm.state("int_state", .{
            hsm.entry(integerDataEntry)
        }),
        hsm.state("string_state", .{
            hsm.entry(stringDataEntry)
        }),
        hsm.state("bool_state", .{
            hsm.entry(booleanDataEntry)
        })
    });
    ...
    try string_event.putData("message", &string_value);
...
test "Multiple data fields in single event" {
    const model = comptime hsm.define("MultiDataTest", .{
        hsm.initial(hsm.target("processor")),
        hsm.state("processor", .{
            hsm.transition(.{ hsm.on("multi_data"), hsm.effect(multiDataEffect), hsm.target("result") })
        }),
        hsm.state("result", .{})
    });
    ...
    try multi_event.putData("text", &text_value);
...
test "Complex structured data in events" {
    const model = comptime hsm.define("ComplexDataTest", .{
        hsm.initial(hsm.target("waiting")),
        hsm.state("waiting", .{
            hsm.transition(.{ hsm.on("complex_event"), hsm.effect(complexDataEffect), hsm.target("processed") })
        }),
        hsm.state("processed", .{})
    });
...
test "Event data in guard conditions" {
    const model = comptime hsm.define("GuardDataTest", .{
        hsm.initial(hsm.target("evaluator")),
        hsm.state("evaluator", .{
            hsm.transition(.{ hsm.on("check_value"), hsm.guard(valueGuard), hsm.target("above_threshold") }),
            hsm.transition(.{ hsm.on("check_value"), hsm.target("below_threshold") }),
            hsm.transition(.{ hsm.on("check_string"), hsm.guard(stringLengthGuard), hsm.target("long_string") }),
            hsm.transition(.{ hsm.on("check_string"), hsm.target("short_string") })
        }),
        hsm.state("above_threshold", .{}),
        hsm.state("below_threshold", .{}),
        hsm.state("long_string", .{}),
        hsm.state("short_string", .{})
    });
...
test "Event data propagation through state hierarchy" {
    ...
    const model = comptime hsm.define("HierarchyDataTest", .{
        hsm.initial(hsm.target("parent")),
        hsm.state("parent", .{
            hsm.entry(hierarchyDataEntry),
            ...
            hsm.state("child", .{
                ...
                hsm.state("grandchild", .{
                    hsm.entry(hierarchyDataEntry),
                    hsm.transition(.{ hsm.on("data_event"), hsm.effect(hierarchyDataEntry), hsm.target("../../other_child") })
                })
            }),
            hsm.state("other_child", .{
                hsm.entry(hierarchyDataEntry)
            })
        })
    });
...
test "Event data modification and chaining" {
    ...
    const model = comptime hsm.define("ChainDataTest", .{
        hsm.initial(hsm.target("chain_start")),
        hsm.state("chain_start", .{
            hsm.transition(.{ hsm.on("chain"), hsm.effect(.{ ChainEffect.func, ChainEffect.func, ChainEffect.func }), hsm.target("chain_end") })
        }),
        hsm.state("chain_end", .{})
    });
...
test "Event data with null and missing fields" {
    ...
    const model = comptime hsm.define("SafeDataTest", .{
        hsm.initial(hsm.target("safe_state")),
        hsm.state("safe_state", .{
            hsm.transition(.{ hsm.on("safe_event"), hsm.effect(SafeDataAccess.func), hsm.target("result") })
        }),
        hsm.state("result", .{})
    });
