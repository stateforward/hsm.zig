...
test "Single entry function execution" {
    const model = comptime hsm.define("SingleEntryTest", .{
        hsm.initial(hsm.target("state1")),
        hsm.state("state1", .{
            hsm.entry(firstEntry),
            hsm.transition(.{ hsm.on("next"), hsm.target("state2") })
        }),
        hsm.state("state2", .{
            hsm.entry(secondEntry)
        })
    });
...
test "Multiple entry functions execution order" {
    const model = comptime hsm.define("MultipleEntryTest", .{
        hsm.initial(hsm.target("setup_state")),
        hsm.state("setup_state", .{
            hsm.entry(.{ initializingEntry, setupEntry, configureEntry }),
            hsm.transition(.{ hsm.on("ready"), hsm.target("running_state") })
        }),
        hsm.state("running_state", .{
            hsm.entry(.{ firstEntry, secondEntry })
        })
    });
...
test "Entry functions with event data access" {
    ...
    const model = comptime hsm.define("EntryDataTest", .{
        hsm.initial(hsm.target("start")),
        hsm.state("start", .{
            hsm.transition(.{ hsm.on("with_data"), hsm.target("data_state") })
        }),
        hsm.state("data_state", .{
            hsm.entry(dataEntry)
        })
    });
...
test "Entry functions in hierarchical states" {
    const model = comptime hsm.define("HierarchicalEntryTest", .{
        ...
        hsm.state("parent", .{
            ...
            hsm.state("child", .{
                hsm.entry(secondEntry),
                hsm.transition(.{ hsm.on("next"), hsm.target("../sibling") })
            }),
            hsm.state("sibling", .{
                hsm.entry(thirdEntry)
            })
        })
    });
...
test "Entry functions with context cancellation check" {
    ...
    const model = comptime hsm.define("EntryCancellationTest", .{
        hsm.initial(hsm.target("start")),
        hsm.state("start", .{
            hsm.entry(cancelCheckEntry),
            hsm.transition(.{ hsm.on("next"), hsm.target("next_state") })
        }),
